...
def test(encoder_model, query_dataloader, train_dataloader, result_root, encode, aug_id, batch_size, run_id):
    ...
    result.create_retrieval_file(result_root, encode, aug_id, batch_size, run_id)
...
def main():
    ...
    train_dataset = Equation(root, encode=encode, data_type='train')
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    query_dataset = Equation(root, encode=encode, data_type='query')
    ...
    aug = {1: A.FeatureMasking(pf=0.3), 
        ...
        6: SwapNodeContent(pe=0.3, swap_dict=swap_dict, char_emb=char_emb),
        7: A.RandomChoice([ A.NodeDropping(pn=aug_p),
                            A.FeatureMasking(pf=aug_p),
                            A.EdgeAttrMasking(pf=aug_p),
                            A.EdgeRemoving(pe=aug_p),
                            SwapNodeContent(pe=aug_p, swap_dict=swap_dict, char_emb=char_emb)], 1),
    }
    ...
    test(encoder_model, query_dataloader, judge_dataloader, result_root, encode, aug_id, batch_size, run_id)
